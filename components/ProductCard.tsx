    ...
    >
      ...
      <div className="relative w-full aspect-[3/4] overflow-hidden rounded-xl md:rounded-2xl bg-[#F2F0EB] shadow-sm group-hover:shadow-xl group-hover:shadow-primary/5 transition-all duration-500">
        ...
        <div className="absolute top-2 left-2 right-2 md:top-3 md:left-3 md:right-3 flex justify-between items-start z-10">
          ...
          <button 
            ...
            className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-white/80 backdrop-blur-sm flex items-center justify-center text-primary hover:bg-accent hover:text-white transition-all duration-300 shadow-sm md:translate-x-2 md:opacity-0 md:group-hover:translate-x-0 md:group-hover:opacity-100"
        ...
        <div className="hidden md:block absolute inset-x-0 bottom-0 p-4 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 cubic-bezier(0.19, 1, 0.22, 1) will-change-transform">
          <button 
            onClick={(e) => {
              e.stopPropagation();
              onAddToCart(product);
            }}
            className="w-full bg-white/95 backdrop-blur-md border border-white/50 text-primary py-3.5 rounded-xl flex items-center justify-center gap-2 hover:bg-primary hover:text-white transition-all duration-300 shadow-lg group/btn"
          >
      ...
      <div className="pt-3 md:pt-4 px-0 md:px-1 flex flex-col gap-0.5 md:gap-1">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-1 bg-[#F2F0EB] px-1.5 py-0.5 rounded">
        ...
        {/* Product Name - Scaled down for mobile grid */}
        <h3 className="font-serif text-[13px] md:text-[17px] leading-tight text-primary group-hover:text-accent transition-colors duration-300 line-clamp-2 min-h-[2.2em]">
          {product.name}
        </h3>
        
        {/* Price Section */}
        <div className="flex flex-wrap items-baseline gap-1.5 md:gap-2 mt-0.5">
        ...
        {/* Scarcity - Hide on very small screens/mobile grid if too long, or truncate */}
        {product.scarcityText ? (
          <div className="mt-1 md:mt-2 flex items-center gap-1.5 hidden sm:flex">
            <div className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></div>
            <p className="text-[10px] font-medium text-red-700/80 leading-none truncate">
              {product.scarcityText}
            </p>
          </div>
        ) : (
          <div className="mt-1 md:mt-2 h-3 md:h-4 hidden sm:block"></div>
        )}
      </div>
    </div>
  );
